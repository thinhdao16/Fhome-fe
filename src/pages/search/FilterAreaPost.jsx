...
const FilterAreaPost = ({ children }) => {
  const [value, setValue] = useState(0);
  const [rating, setRating] = useState(0); // initial rating value

  const { filterAreaPosting } = useContext(DataContext);
  console.log(filterAreaPosting);
  ...
  function handleCommentPost(event, id) {
    ...
    const index = filterAreaPosting.findIndex((item) => item._id === id);
    const idDataPost = filterAreaPosting[index];
    ...
    <DataContext.Provider value={{ selectedPost }}>
      ...
      <div className="px-5">
        ...
        <div className="container">
          {Array.isArray(filterAreaPosting) &&
            filterAreaPosting
              .sort((a, b) => {
                return (
